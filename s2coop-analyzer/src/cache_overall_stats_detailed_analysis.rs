...
impl CacheOverallStatsDetailedAnalysis {
    pub fn run(
        ...
        let Some(account_dir) = args
            .account_dir
            .clone()
            .or_else(CacheAnalysisPaths::resolve_account_dir)
        else {
            return Ok(
                "skipping exact parity test: no SC2 account directory configured".to_string(),
            );
        };
        ...
        let summary =
            DetailedReplayAnalyzer::analyze_full_detailed(&config, &resources, logger, &runtime)?;
        ...
        let generated_payload = JsonPayloadReader::read(summary.output_file())?;
...
impl CacheAnalysisPaths {
    ...
    fn default_generated_output() -> PathBuf {
        Self::runtime_root().join("cache_overall_stats.json")
    }
...
impl JsonPayloadReader {
    fn read(path: &Path) -> Result<Value, TestCacheOverallStatsDetailedAnalysisError> {
...
struct PythonValueFormat;

impl PythonValueFormat {
    ...
    fn is_number(value: &Value) -> bool {
        Self::number_value(value).is_some()
    }
    ...
    fn type_name(value: &Value) -> &'static str {
        match value {
            Value::Null => "NoneType",
            Value::Bool(_) => "bool",
            Value::Number(_) => Self::numeric_type_name(value).unwrap_or("float"),
            Value::String(_) => "str",
            Value::Array(_) => "list",
            Value::Object(_) => "dict",
        }
    ...
    fn general_float(value: f64) -> String {
        ...
        if !(1e-4..1e16).contains(&abs_value) {
            ...
            let trimmed_mantissa = Self::trim_trailing_zeroes(mantissa);
        ...
        Self::trim_trailing_zeroes(&format!("{value:.16}"))
    }
    ...
    fn repr(value: &Value) -> String {
        match value {
            ...
            Value::String(text) => Self::string_repr(text),
            Value::Array(items) => {
                let parts = items.iter().map(Self::repr).collect::<Vec<String>>();
                format!("[{}]", parts.join(", "))
            }
            Value::Object(map) => {
                let parts = map
                    .iter()
                    .map(|(key, inner)| {
                        format!("{}: {}", Self::string_repr(key), Self::repr(inner))
                    })
                    .collect::<Vec<String>>();
                format!("{{{}}}", parts.join(", "))
            }
...
struct JsonPathDisplay;

impl JsonPathDisplay {
...
struct CachePayloadDiff;

impl CachePayloadDiff {
    fn collect_numeric_type_differences(
        ...
        if PythonValueFormat::is_number(expected) && PythonValueFormat::is_number(actual) {
            let Some(expected_value) = PythonValueFormat::number_value(expected) else {
                return;
            };
            let Some(actual_value) = PythonValueFormat::number_value(actual) else {
                return;
            };
            if expected_value == actual_value
                && PythonValueFormat::numeric_type_name(expected)
                    != PythonValueFormat::numeric_type_name(actual)
            {
                out.push(NumericTypeDiff {
                    path: JsonPathDisplay::root_or_path(path),
                    expected_type: PythonValueFormat::numeric_type_name(expected)
                        .unwrap_or("number"),
                    actual_type: PythonValueFormat::numeric_type_name(actual).unwrap_or("number"),
                    expected_value: PythonValueFormat::repr(expected),
                    actual_value: PythonValueFormat::repr(actual),
                });
            }
            return;
        }
            ...
            (Value::Array(expected_items), Value::Array(actual_items)) => {
                    ...
                    expected_items.iter().zip(actual_items.iter()).enumerate()
                {
                    Self::collect_numeric_type_differences(
                        expected_item,
                        actual_item,
                        &JsonPathDisplay::index_path(path, index),
                        out,
                        limit,
                    );
                    if out.len() >= limit {
    ...
    fn format_numeric_type_differences(diffs: &[NumericTypeDiff]) -> String {
        ...
        let mut lines = vec![
            "Numeric type-only differences (same numeric value, different number type):"
                .to_string(),
        ];
        for diff in diffs {
            lines.push(format!(
    ...
    fn collect_first_field_differences(
        ...
        if PythonValueFormat::is_number(expected) && PythonValueFormat::is_number(actual) {
            ...
            out.push(FieldDifference {
                path: JsonPathDisplay::root_or_path(path),
                expected: PythonValueFormat::repr(expected),
                actual: PythonValueFormat::repr(actual),
        ...
        match (expected, actual) {
            (Value::Object(expected_map), Value::Object(actual_map)) => {
                ...
                for key in keys {
                    let next_path = JsonPathDisplay::key_path(path, &key);
                    match (expected_map.get(&key), actual_map.get(&key)) {
                        ...
                        (Some(expected_value), None) => out.push(FieldDifference {
                            path: next_path,
                            expected: PythonValueFormat::repr(expected_value),
                            actual: PythonValueFormat::string_repr("<missing>"),
                            abs_delta: None,
                        }),
                        (None, Some(actual_value)) => out.push(FieldDifference {
                            path: next_path,
                            expected: PythonValueFormat::string_repr("<missing>"),
                            actual: PythonValueFormat::repr(actual_value),
                            abs_delta: None,
                        }),
                        (None, None) => {}
                    }
            ...
            _ => {
                let same_type =
                    PythonValueFormat::type_name(expected) == PythonValueFormat::type_name(actual);
    ...
    fn format_report(
        ...
        let (Value::Array(expected_replays), Value::Array(actual_replays)) =
            ...
            return format!(
                ...
                PythonValueFormat::type_name(expected_payload),
                PythonValueFormat::type_name(actual_payload)
            );
        };
                ...
                for field_difference in field_differences {
                    match field_difference.abs_delta {
                        Some(delta) => lines.push(format!(
                            ...
                            PythonValueFormat::general_float(delta)
                        )),
    ...
    fn compare_with_report(
        ...
        let actual_payload: Value = serde_json::from_slice(&actual_bytes).map_err(|error| {
            TestCacheOverallStatsDetailedAnalysisError::ParseFailed(
        ...
        if significant_differences.is_empty() {
            ...
            if !numeric_type_differences.is_empty() {
                lines.push(Self::format_numeric_type_differences(
                    &numeric_type_differences,
                ));
            }
