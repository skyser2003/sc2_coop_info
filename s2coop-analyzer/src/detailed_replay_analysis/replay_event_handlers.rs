...
fn increment_i64_key(container: &mut [i64], key: i64, delta: i64) {
    if let Some(payload) = IndexedDelta::new(key, delta) {
        apply_indexed_delta(container, payload);
    }
}
...
pub(super) fn replay_handle_unit_born_or_init_event_fields(
        ...
        && event.creator_unit_id.is_some()
    {
        if let Some(creator_id) = event.creator_unit_id {
            if let Some(creator_row) = unit_dict.get(&creator_id) {
                ...
                if creator_type == "BroodlingEscort" || creator_type == "BroodlingEscortStetmann" {
    ...
    if let Some(revival_target) = revival_types.get(unit_type) {
        if (control_pid == 1 || control_pid == 2) && game_time > start_time + 1.0 {
            ...
            if control_pid == ally_player {
                update_unit_count(unit_type_dict_ally, revival_target.as_str(), 1, 1, 0);
            }
        }
    }
    ...
    if let Some(predecessor) = primal_combat_predecessors.get(unit_type) {
        ...
        if control_pid == ally_player {
            update_unit_count(unit_type_dict_ally, predecessor.as_str(), 0, -2, 0);
        }
    }
...
pub(super) fn replay_handle_unit_type_change_event_fields(
    ...
    if map_name.contains("Scythe of Amon") && control_pid == 11 && unit_type == "WarpPrismPhasing" {
        bonus_timings.push(gameloop as f64 / 16.0 - start_time);
    }
        ...
        if !do_not_count_kills.contains(killed_unit_type) {
            ...
            } else if amon_players.contains(&killer) && !(losing_player == 1 || losing_player == 2)
            {
                ...
                if ally_leave_time < end_time * 0.5 {
                    increment_i64_key(killcounts, main_player, 1);
                    ally_kills += 1;
                } else {
                    increment_i64_key(killcounts, killer, 1);
                }
            } else {
                increment_i64_key(killcounts, killer, 1);
            }
...
pub(super) fn replay_handle_unit_died_detail_event_fields(
    ...
    {
        increment_i64_key(killbot_feed, losing_player, 1);
    }
        ...
        && amon_players.contains(&losing_player)
    {
        ...
        if killed_unit_type == "ProtossFrigate" {
            increment_nested_player_count(custom_kill_count, "shuttles", killer, 1);
            ...
            increment_nested_player_count(custom_kill_count, "missilecommand", killer, 1);
        }
    }
            ...
            .unwrap_or(false)
    {
                ...
                .unwrap_or(false)
        {
            ...
            increment_nested_player_count(custom_kill_count, "minesweeper", losing_player, 1);
        }
    }
        ...
        if let Some(killer) = killing_player {
                ...
                .and_then(|entry| entry.as_ref())
            {
                if event.gameloop as f64 / 16.0 - *last_time < 9.0 {
                    update_unit_count(unit_type_dict_amon, last_type.as_str(), 0, 0, 1);
                }
            }
        }
    }
    ...
    let ignore_count = usize::try_from(losing_player)
        ...
        .copied()
        .unwrap_or_default();
    if (killed_unit_type == "HighTemplar" || killed_unit_type == "DarkTemplar") && ignore_count > 0
    {
        increment_i64_key(dt_ht_ignore, losing_player, -1);
        return update;
    }

    let event_x = event.event_x;
    let event_y = event.event_y;
    let rounded_bonus = round_to_digits_half_even(game_time - start_time, 0);
    ...
    if bonus_triggered {
        if map_name.contains("Cradle of Death") {
            let value = round_to_digits_half_even(game_time - start_time - 8.0, 0);
            bonus_timings.push(value);
        } else {
            let value = round_to_digits_half_even(game_time - start_time, 0);
            bonus_timings.push(value);
        }
    }
