...
fn runtime_flags_follow_active_settings_before_save() {
    let previous_settings = super::read_settings_file();

    super::replace_active_settings(&json!({
        "start_minimized": true,
        "minimize_to_tray": false,
    }));
    let disabled_flags = super::overlay_info::parse_runtime_flags();
    ...
    let enabled_flags = super::overlay_info::parse_runtime_flags();
    assert!(!enabled_flags.start_minimized);
    assert!(enabled_flags.minimize_to_tray);

    super::replace_active_settings(&previous_settings);
}
